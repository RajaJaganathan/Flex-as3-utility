	...
	public class StringUtil {
		...
		public static function naturalCompare(a:String, b:String):int {
			...
			while (true) {
				...
				while (StringUtil.characterIsWhitespace(ca) || ca == "0") {
					if (ca == "0") {
						nza++;
					} else {
				...
				while (StringUtil.characterIsWhitespace(cb) || cb == "0") {
					if (cb == "0") {
						nzb++;
					} else {
						// only count consecutive zeroes
				...
				if (StringUtil.characterIsDigit(ca) && StringUtil.characterIsDigit(cb)) {
		...
		private static function compareRight(a:String, b:String):int {
			...
			for (; ; ia++, ib++) {
				...
				if (!StringUtil.characterIsDigit(ca) && !StringUtil.characterIsDigit(cb)) {
					return bias;
				} else if (!StringUtil.characterIsDigit(ca)) {
					return -1;
				} else if (!StringUtil.characterIsDigit(cb)) {
					return +1;
				} else if (ca < cb) {
					if (bias == 0) {
						bias = -1;
		...
		public static function indexOfIgnoreCase(str:String, searchStr:String):int {
